...
class SNConv2d(nn.modules._ConvNd):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True):
        kernel_size = nn.utils._pair(kernel_size)
        stride = nn.utils._pair(stride)
        padding = nn.utils._pair(padding)
        dilation = nn.utils._pair(dilation)
        super(SNConv2d, self).__init__(
            in_channels, out_channels, kernel_size, stride, padding, dilation,
            False, nn.utils._pair(0), groups, bias)
